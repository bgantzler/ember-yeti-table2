...
module('Integration | Component | yeti-table (general)', function (hooks) {
  ...
  test('can add arbitrary attributes to columns and cells', async function (assert) {
    ...
    assert
      ...
      .hasAttribute('data-column', 'test-column');
    assert
      .dom('thead tr:nth-child(1) th:nth-child(1)')
    ...
    assert
      .dom('tbody tr:nth-child(1) td:nth-child(1)')
      .hasAttribute('data-cell', 'test-cell');
    assert
      .dom('tbody tr:nth-child(1) td:nth-child(1)')
      .hasClass('column-class');
  ...
  test('yielded columns have a name property equal to the @name argument, overriding the default', async function (assert) {
    ...
    assert.dom('tfoot > tr > td').exists({ count: 3 });
    assert
      .dom('tfoot tr:nth-child(1) td:nth-child(1)')
      .hasText('First name yo!');
    assert
      .dom('tfoot tr:nth-child(1) td:nth-child(2)')
      .hasText('Last name yo!');
